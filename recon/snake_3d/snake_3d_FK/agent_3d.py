...
def train(num_episode = 2000,load = True):
    game = HybridGame()
    if load:
        agent = load_model(checkpoint_path='model_3d/model_701_9.pth') 
    else:
        agent = HybridAgent()
    record = 0
    total_score = 0
    ...
    try:
        ...
        for episode in trange(num_episode, desc="Training Progress"):
            ...
            for step in range(63):
                ...
                # Get action from agent (one-hot vector), then convert to index and map to a 3D direction
                final_move = agent.get_action(state_old)
                action_index = np.argmax(final_move).item()
                head, _ = game.robot.apply_action(action_index)
                
                # Execute the action in the environment
                reward, done, score = game.play_step(head)
