...
class HybridGame:
    ...
    def play_step(self, action_idx):
        ...
        if self.food:
            closest_food = min(self.food,
                               key=lambda f: np.linalg.norm(f - self.robot.head))

        else:
            closest_food = self.robot.head
        prev_dist = np.linalg.norm(self.robot.head - closest_food)
        ...
        new_dist = np.linalg.norm(head - closest_food)
